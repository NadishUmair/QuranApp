...
const AdminCourse = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const data = await axios.post("http://localhost:8080/api/users/addcourse", form);
  ...
  const deleteCourse = async (id) => {
    try {
      const deletecourse = await axios.delete(`http://localhost:8080/api/users/deletecourse/${id}`);
